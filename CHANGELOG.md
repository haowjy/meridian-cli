...
- Normalize retained model defaults before projecting native-argument controls;
  whitespace-only canonical/default identities cannot authorize raw-argument suppression.
...
- Retain and shield single-flight native exit observation across waiter
  cancellation; retries preserve completed terminal evidence without reclosing.
...
- OpenCode spawns no longer hang on an unanswered `permission.asked`/`permission.v2.asked`. The V1/V2 connection now routes stream permission asks to the injected request handler, so spawned agents auto-reject and managed primaries surface them; `respond_request` posts the version's reply endpoint (`/session/{id}/permissions/{permissionID}` for V1, `/api/session/{id}/permission/{requestID}/reply` for V2). Injected policy events (`request/opened`, `request/resolved`) are multiplexed into `events()` ahead of an idle SSE read, and unanswered requests are cancelled on cleanup.
- An OpenCode permission ask answered natively in the TUI (a `permission.replied`/`permission.v2.replied` stream event) is now observed instead of ignored: the connection clears the pending request, journals `request/resolved` with the mapped decision (`once`/`always` → `accept`, `reject` → `reject`, raw `reply` retained) and releases its liveness key. A natively-answered managed primary can no longer mask a later real stream stall.
...
- The cancel path applies the same execution-fact precedence: `complete_execution` folds the durable cancel intent into its runner facts instead of letting a second, durable-first rule override the result, and reaper/reconcile callers thread runner-exit evidence through the shared resolver. A `spawn cancel`-ed permission hang with prose now ends `cancelled`, not `succeeded`.
- OpenCode permission dispatch no longer blocks the SSE drain: an unanswered reply is dispatched in a bounded background task (a stalled POST can no longer freeze `events()` and its liveness), and injected `request/opened`/`request/resolved` events keep FIFO order. An unknown permission request id now fails fast instead of replying against the session id.
- The OpenCode connection dispatcher forwards request-failure notifications to its version transport, so failed OpenCode replies are journaled `failed` like Codex.
...
- A managed-primary launcher no longer dies on a rapid terminal-close signal burst: a repeat of the same signal arriving at least `escalate_repeat_grace_secs` (default 1s) after the first is treated as a force-quit, so a burst within the grace is routed to the cancel callback instead.
...
- Read mixed ZIP archives per record so a legacy history.jsonl session co-archived with a native snapshot remains readable.
- Thread transcript read budget and reserved-frame checks through providers instead of type-switching in the shared reader.
