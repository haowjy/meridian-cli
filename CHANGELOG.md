...
- `meridian spawn -h` (agent mode) now carries the full operational methodology that lived in the `meridian-spawn` skill, so the skill can be retired: core loop + hard drain rule, prompt-file rationale, profile-vs-model, folder-over-bundle context refs, `--goal` as exit state, parallel/anti-poll discipline, and the continue/fork/fork-fresh/from reuse decision. The always-on contract (`spawn_guidance.py`) gained the load-bearing drain invariant ("every `--bg` spawn must be drained with `meridian spawn wait` before you respond, start dependent work, or end your turn; un-waited background spawns are lost"); leaf help gained one-line behavioral boundaries on `spawn inject` (forward real user direction only), `spawn files` (commit pattern), and `--prompt-var`.
- Agent-mode `meridian spawn -h` is now progressively disclosed via a `--advanced` tier. The default view is lean (~85 lines, down from ~160): a `Quick start:` block carrying the core loop *plus* the pre-launch decisions an agent must make before composing its first spawn (prompt-file vs inline, context-shaping, `--goal` as exit state, the continue/fork/fork-fresh/from reuse choice, and the inspect/steer/search entry points), the six core commands (`wait`/`show`/`list`/`inject`/`files`/`cancel`), and the core flags only. `meridian spawn -h --advanced` reveals the full methodology, every agent command, and an `Advanced` panel of less-common flags (`--prompt-var`, `--skills`, `--task-dir`, `--dry-run`, `--timeout`, `--approval`, `--effort`, `--sandbox`, `--yolo`, `--env`, etc.). Tiering is agent-mode only — human `spawn -h` stays complete (the advanced flags render in an always-shown `Advanced` panel), and `--advanced` is only honored on tiered groups (`meridian doctor -h --advanced` errors as an unknown option). New `meridian/cli/help_tiers.py` holds the shared cyclopts `Advanced` parameter group; its visibility is a pure function of render mode set inside the help-curation transaction (`agent_help.apply_agent_help`), revealed for human/`--advanced` and hidden only in the agent lean view.
...
- Root `--help` (agent and human) now renders from one shared, composable source instead of two hand-authored blocks. Single `GROUP_DESCRIPTIONS` source (kills the drift where e.g. `spawn` had two different blurbs), a shared aligned-table renderer, and a consistent section order/usage line. The two views now differ only where intended: agent gets orientation prose, a curated command set, and a quick-start; human gets the full command set and the version footer. Agent help gains a compact `Options` block (incl. `-m`/`-C`); the human launch/resume block is now column-aligned instead of double-spaced. `AGENT_DESCRIPTION_OVERRIDES` is the per-view override seam.
- Group and leaf `--help` no longer repeat root-level or group-level blocks. The root "Primary launch/resume" epilogue used to inherit (via cyclopts `help_epilogue` resolution) into every group's help, and group examples/notes inherited into every leaf (`spawn show -h` repeated spawn's whole Examples block). Group examples + agent notes now render in the group **description** (only on the group page); all `help_epilogue` are emptied so nothing inherits. A new single source `meridian/cli/help_content.py` (`GROUPS`) feeds root help, group descriptions (`app_tree`), and agent curation (`agent_help`), collapsing the previous three drifting description sources. Agent curation now covers all groups uniformly; empty parameter help filled for `doctor` (`--prune`/`--global`/`--kill-orphans`).
