...
- Agent-mode `meridian spawn -h` is now progressively disclosed via a `--advanced` tier. The default view is lean (~70 lines, down from ~160): a `Quick start:` core-loop summary, the six core commands (`wait`/`show`/`list`/`inject`/`files`/`cancel`), and the core flags only. `meridian spawn -h --advanced` reveals the full methodology, every agent command, and an `Advanced` panel of less-common flags (`--prompt-var`, `--skills`, `--task-dir`, `--dry-run`, `--timeout`, `--approval`, `--effort`, `--sandbox`, `--yolo`, `--env`, etc.). Tiering is agent-mode only — human `spawn -h` stays complete (the advanced flags render in an always-shown `Advanced` panel). New `meridian/cli/help_tiers.py` holds the shared cyclopts `Advanced` parameter group; visibility is toggled per help render and restored, like the existing subcommand curation.
