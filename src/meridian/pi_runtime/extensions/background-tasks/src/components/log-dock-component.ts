...
export class LogDockComponent implements Component {
  ...
  private renderCollapsed(width: number): string[] {
    ...
    const processes = this.host.listSync();
    const lineWidth = getCollapsedDockLineWidth(width);
    const padLine = (content: string) =>
      renderCollapsedDockLine(content, width);

    if (processes.length === 0) {
      return [renderPanelRule(lineWidth, theme), padLine(dim("No tasks"))];
    }

    const running = processes.filter((p) => p.isLive);
    const finished = processes.filter((p) => !p.isLive);
    ...
    if (running.length > 0) {
      const lastLogs = this.host.getCombinedOutput(running[0].id, 1);
  ...
  private renderOpen(width: number): string[] {
    ...
    const logFiles = this.host.getLogFiles(targetProc.id);
    if (!logFiles) {
      return [
        renderPanelTitleLine("Process Logs", width, theme),
