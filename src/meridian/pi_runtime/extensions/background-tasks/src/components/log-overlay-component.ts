...
export class LogOverlayComponent implements Component {
  private tui: TUI;
  private theme: Theme;
  private host: TaskPanelHost;
  private done: () => void;

  private processes: PanelEntry[] = [];
  private tabIndex = 0;
  ...
  constructor(opts: LogOverlayOptions) {
    ...
    this.unsubscribeManager = this.host.onEvent(() => {
  ...
  private async refreshProcesses(initialProcessId?: string): Promise<void> {
    const next = await this.host.list();
    if (next.length === 0) {
      this.close();
      return;
    }
    this.host.setSyncEntries(next);
    this.processes = this.sortProcesses(next);
    if (initialProcessId) {
      const idx = this.processes.findIndex((p) => p.id === initialProcessId);
      if (idx >= 0) {
        this.tabIndex = idx;
      }
    } else {
  ...
  private sortProcesses(list: PanelEntry[]): PanelEntry[] {
  ...
  private getViewer(proc: PanelEntry): LogFileViewer | null {
    let viewer = this.viewers.get(proc.id);
    if (!viewer) {
      const logFiles = this.host.getLogFiles(proc.id);
      if (!logFiles?.combinedFile) {
        return null;
      }
  ...
  private currentProcess(): PanelEntry | null {
  ...
  private renderTabBar(innerWidth: number): string {
    ...
    const coloredIcon = (proc: PanelEntry): string => {
