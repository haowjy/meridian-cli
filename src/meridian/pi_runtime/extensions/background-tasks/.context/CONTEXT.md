...
- **In-chat hint** (`meridian:foreground-bash-hint`): once per task for foreground **`$`** or tracked agent **`bash`**. Text in `details.hintText` (empty `content`); `triggerTurn: false`, `excludeFromContext: true`. User: `/ps`, ctrl+b, `&`; agent: see `/ps`. Not for trailing **`&`** or detached bash.
