...
- **In-chat hint** (`meridian:foreground-bash-hint` via `pi.sendMessage` + `registerMessageRenderer`): once per foreground **`$`** task — **`/ps to manage tasks · ctrl+b to run in background`** (`triggerTurn: false`). Not posted for agent **`bash`**, trailing **`&`**, or detached runs.
