...
- **In-chat hint** (`meridian:foreground-bash-hint`): once per foreground **`$`** task — `/ps to manage tasks · ctrl+b to run in background`. Text in `details.hintText` (empty `content`); `triggerTurn: false`, `excludeFromContext: true`. Not for trailing **`&`** (already detached).
