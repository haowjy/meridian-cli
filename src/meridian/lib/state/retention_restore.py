...
def restore_archive(root: Path, archive_path: Path, refs: tuple[str, ...]) -> tuple[str, ...]:
    ...
    restored: list[str] = []
    with lock_file(root / "history-archives/archive.lock"), lock_file(changes.mutation_lock):
        scan = list_spawns(root)
        if scan.quarantines:
            raise ValueError("Cannot resolve restore conflicts with quarantined records")
        by_history = {row.history_id: row for row in scan.records if row.history_id}
        plans = root / "history-archives" / "restores"
        for record in selected:
            plan_path = plans / f"{record.history_id}.json"
            existing = by_history.get(record.history_id)
            ...
            with lock_file(source_lock):
                ...
                if current is not None:
                    ...
                    with zipfile.ZipFile(archive_path) as archive:
                        for member in record.files:
                            ...
                            with (
                                ...
                                atomic_replace(
                                    target, mode="wb", encoding=None, permissions=0o600
                                ) as outgoing,
                            ):
                    ...
                    atomic_write_text(
                        stage / "state.json", record_to_stored_state(state).model_dump_json()
                    )
            ...
            plan_path.unlink()
            restored_state = read_state(root / "spawns", plan.local_id, include_prompt=False)
            assert restored_state is not None
            by_history[record.history_id] = restored_state
