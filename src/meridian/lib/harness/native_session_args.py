...
class NativeSessionArgsNormalizer(Protocol):
    """Normalize one raw argument vector using a harness-specific grammar."""

    def __call__(self, args: tuple[str, ...], /) -> NormalizedNativeSessionArgs: ...


def normalize_native_session_args(
    args: tuple[str, ...],
    normalizer: NativeSessionArgsNormalizer | Callable[
        [tuple[str, ...]], NormalizedNativeSessionArgs
    ] | None,
