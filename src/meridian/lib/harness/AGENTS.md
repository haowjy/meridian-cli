...
Only owned signals count: a connection response or the process's own artifacts,
then an already-known ID. Cwd, timestamps, logs, and newest-file scans never
establish tracked chat identity. Must not mutate adapter-instance state. Returned
observations cannot overwrite a bound identity. `plan_native_identity()` provides
pre-exec assignments where supported; child-store resolution uses the final child
environment.
