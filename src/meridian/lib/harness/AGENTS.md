...
- `semantics.py` — the generic `HarnessSemantics` port, closed semantic union, and
  dispatch-by-`HarnessId` lookup. Event-name tables and payload resolvers live beside
  each adapter's `HarnessBundle` registration.
