...
doubt skips, and a failure on one spawn is recorded without stopping the pass.
Explicit only; automatic maintenance never calls this.

Multi-attempt runs are safe too: Meridian reads no runner bytes for any
attempt, and each attempt's turns stay in the harness's own native files.
"""
...
def runner_stream_files(root: Path, spawn_id: str) -> tuple[Path, ...]:
    ...
    for base in (root / "spawns" / spawn_id, root / "artifacts" / spawn_id):
        ...
        for directory in (base, *attempts):
            for name in RETIRED_RUNNER_STREAM_FILENAMES:
...
class _Verdict:
    """Why a spawn keeps its runner stream, or the native sources that replace it."""

    skip: str | None = None
    sources: tuple[Path, ...] = ()


def _native_sources(row: SpawnRecord, sessions: Mapping[str, SessionRecord]) -> _Verdict:
    try:
        return _Verdict(sources=tuple(source.path for source in resolve_run_sources(row, sessions)))
    except NativeSessionUnavailable as exc:
        return _Verdict(skip=exc.reason)
...
def _judge(
    ...
    if any(
        ...
        return _Verdict(skip="live_scope")
    return _native_sources(row, sessions)
...
def _prune(root: Path, *, apply: bool, after_days: int) -> RunnerHistoryPrune:
    ...
    rows = sorted(scan.records, key=lambda row: (len(row.id), row.id))
    sessions = {
    ...
    for row in rows:
        try:
            ...
            if not files:
                continue
            verdict = _judge(root, row, sessions, cutoff)
            if verdict.skip is not None:
                skipped.setdefault(verdict.skip, []).append(row.id)
                skipped_bytes[verdict.skip] = skipped_bytes.get(verdict.skip, 0) + _size(files)
            elif not apply:
                pruned.append(_pruned(root, row, files))
            elif (done := _unlink_if_unchanged(root, row, sessions)) is not None:
                pruned.append(done)
            else:
                errors.append(f"{row.id}: record or native source changed since planning; kept")
        except Exception as exc:  # One spawn's failure never stops the pass.
    ...
    return RunnerHistoryPrune(
        ...
        quarantined=tuple(report.spawn_id for report in scan.quarantines),
        errors=tuple(errors),
    )


def _unlink_if_unchanged(
    root: Path, row: SpawnRecord, sessions: Mapping[str, SessionRecord]
) -> PrunedSpawn | None:
    """Unlink under the spawn aggregate lock once the record and natives still hold.

    Per-file unlinks make reruns converge.
    """
    planned = row.model_dump(exclude={"prompt"})
    result: PrunedSpawn | None = None

    def unlink() -> None:
        nonlocal result
        files = runner_stream_files(root, row.id)
        result = _pruned(root, row, files)
        for path in files:
    ...
    mutate_published_spawn_artifact(
        ...
        can_mutate=lambda current: (
            ...
            and _native_sources(current, sessions).skip is None
        ),
    )
    return result
