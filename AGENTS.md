...
2. **Single Responsibility at Every Boundary** *(Martin, SRP)*: A component should have one reason to change. God-object symptoms — modules that change for unrelated reasons, classes that accumulate caller-specific flags, functions that mix policy, IO, persistence, and formatting — are refactor triggers, not future work.
3. **Extend, Don't Modify** *(Open/Closed)*: New harness = one adapter file + registration. New package source = one mars config entry. New CLI command = one module. New extension/plugin capability = one explicit seam, not scattered edits through core. If a feature requires editing 10 files, the abstraction is wrong.
