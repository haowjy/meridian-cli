Smoke-test Codex ability to support two Meridian control-plane injection cases without focusing on PTY rendering.
...
Cases to test for Codex:
1. Interrupt current turn, like pressing Escape/cancel, then inject a user message.
...
Suggested angles:
- Use Meridian's current Codex connection/managed app-server path if there is a CLI/API way to inject/cancel.
- Also consider direct Codex app-server API if practical, but do not spend excessive time reverse-engineering.
- We need whether this is possible, what mechanism works, and limitations.

Deliverable:
Write `.meridian/work/terminal-resize-corruption-investigation/codex-injection-smoke.md` with:
