...
def test_parse_transcript_file_history_provider_tolerates_truncated_utf8_tail(
    tmp_path: Path,
) -> None:
    history_path = tmp_path / HISTORY_FILENAME
    valid_line = (
        b'{"seq":0,"byte_offset":0,"event_type":"assistant","harness_id":"codex",'
        b'"payload":{"message":{"content":[{"type":"text","text":"from history"}]}}}\n'
    )
    invalid_tail = b'{"seq":1,"event_type":"assistant","payload":"\xf0\x9f\x92'
    history_path.parent.mkdir(parents=True, exist_ok=True)
    history_path.write_bytes(valid_line + invalid_tail)
