...
def test_replay_preserves_partial_keys_and_rejects_whole_conflicts(
    tmp_path: Path, generation: str, native_id: str | None,
) -> None:
    start = {
        "event": "start", "chat_id": "c1", "harness": "codex", "model": "test",
        "harness_session_id": native_id, "session_instance_id": generation, "started_at": "0",
    }
    events = [
        start,
        {"event": "update", "chat_id": "c1", "session_instance_id": "wrong",
         "harness_session_id": "wrong", "native_store": "/wrong"},
        {"event": "update", "chat_id": "c1", "session_instance_id": generation,
         "harness_session_id": "first", "startup_attempt_id": "attempt"},
        {"event": "update", "chat_id": "c1", "session_instance_id": generation,
         "native_store": "/native", "source": "legacy_import"},
        {"event": "update", "chat_id": "c1", "session_instance_id": generation,
         "harness_session_id": "second", "active_work_id": "rejected"},
    ...
    event = store.SessionUpdateEvent.model_validate({
        "chat_id": "c1", "session_instance_id": "next",
...
def test_same_binding_appends_startup_attempt_link(tmp_path: Path) -> None:
    ...
    try:
        ...
        result = store.update_session_harness_id(
            tmp_path, chat, "first", native_store="/native",
            session_instance_id=before.session_instance_id, startup_attempt_id="retry",
        )
...
def test_equivalent_generation_spelling_keeps_chat_key_for_later_conflicts(tmp_path: Path) -> None:
    start = {
        "event": "start", "chat_id": "c1", "harness": "codex", "model": "test",
        "harness_session_id": None, "session_instance_id": "a", "started_at": "0",
    }
    events = [
        start,
        {**start, "session_instance_id": " a ", "harness_session_id": "first"},
        {"event": "update", "chat_id": "c1", "session_instance_id": "a",
         "active_work_id": "accepted"},
        {"event": "update", "chat_id": "c1", "session_instance_id": "a",
         "harness_session_id": "other", "active_work_id": "rejected"},
    ]
