...
def _contradiction_contender(
    ...
    observe(Path(root), receipt("run", "attempt", "exit", key("/native/repair-race-b"), order=2))
...
def test_native_key_is_store_qualified_and_binding_is_immutable(tmp_path: Path) -> None:
    ...
    root.mkdir()
    left, right = (
        authority.NativeSessionKey(harness="pi", store=s, native_session_id="same")
        for s in ("/native/A", "/native/B")
    )
    begin(root, "run-a", "attempt-a", store=left.store)
    begin(root, "run-b", "attempt-b", store=right.store)
    left_id = observe(root, receipt("run-a", "attempt-a", "entry", left)).chat_id
    right_id = observe(root, receipt("run-b", "attempt-b", "entry", right)).chat_id
    assert left_id != right_id
    assert observe(root, receipt("run-a", "attempt-a", "exit", left, order=2)).chat_id == left_id
    assert session_store.get_native_session_key(root, str(left_id)) == left
    with pytest.raises(ValueError, match="harness/store"):
        observe(root, receipt("run-a", "attempt-a", "entry", right, order=3))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "/x/\x00y",
        "namespace://db/x",
        "namespace:v1://server/db?",
        "namespace:v1://server/db#",
        "namespace:v1://ser\tver/db",
        "namespace:v1://server/line\nbreak",
    ],
)
def test_native_store_identity_rejects_noncanonical_lexical_forms(store: str) -> None:
    with pytest.raises(ValidationError):
        authority.NativeSessionKey(harness="pi", store=store, native_session_id="native")


def test_versioned_native_store_namespace_is_explicit_and_canonical() -> None:
    first = authority.NativeSessionKey(
        harness="pi", store="namespace:v1://server/database", native_session_id="native"
    )
    second = authority.NativeSessionKey(
        harness="pi", store="namespace:v1://other/database", native_session_id="native"
    )
    assert first.store == "namespace:v1://server/database"
    assert first != second
    with pytest.raises(ValidationError):
        authority.NativeSessionKey(
            harness="pi", store="namespace:v1://server/db/../database", native_session_id="native"
        )
...
def test_resume_source_and_attempt_ownership_are_enforced(tmp_path: Path) -> None:
    ...
    observe(root, receipt("source-run", "source-attempt", "entry", pinned))
    ...
    with pytest.raises(ValueError, match="unknown or unstarted"):
        observe(root, receipt("run", "old-attempt", "exit", pinned))
...
def test_old_attempt_receipt_cannot_cross_successor_attempt_boundary(tmp_path: Path) -> None:
    ...
    observe(root, receipt("retrying-run", "attempt-1", "entry", key("/native/stale")))
    begin(root, "retrying-run", "attempt-2")
    with pytest.raises(ValueError, match="superseded attempt"):
        observe(root, receipt("retrying-run", "attempt-1", "exit", key("/native/stale"), order=2))
...
def test_torn_tail_after_live_exit_blocks_unrelated_writer_and_preserves_bytes(
    ...
    observe(root, receipt("run", "attempt", "exit", native_key))
    journal = root / "sessions.jsonl"
...
def test_repair_holds_session_lock_until_replacement_and_contender_acknowledges(
    ...
    observe(root, receipt("run", "attempt", "exit", first))
    journal = root / "sessions.jsonl"
...
def test_complete_final_object_without_newline_is_replayed_before_append(tmp_path: Path) -> None:
    ...
    event = observe(root, receipt("run", "attempt", "entry", key_value))
    journal = root / "sessions.jsonl"
    journal.write_bytes(journal.read_bytes().rstrip(b"\n"))
    assert observe(root, receipt("run", "attempt", "entry", key_value)).chat_id == event.chat_id
...
def test_corrupt_interior_and_schema_invalid_rows_preserve_evidence(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="Unsupported"):
        begin(root, "run", "attempt", transport_scope_id="transport:attempt")
...
def test_unqualified_raw_receipt_and_relative_store_are_rejected() -> None:
    with pytest.raises(ValueError, match="absolute path"):
        authority.NativeSessionKey(
            harness="pi", store="relative/from-cwd", native_session_id="native"
        )
    with pytest.raises(ValidationError):
        authority.BoundaryFact.model_validate(
...
def test_failed_refutation_cannot_be_hidden_by_confirming_read(
    ...
    monkeypatch.setattr(session_store, "fsync_directory", fail_directory_sync)
    with pytest.raises(OSError, match="persistent directory sync failure"):
        observe(root, receipt("run", "attempt", "exit", second, order=2))
    with pytest.raises(OSError, match="persistent directory sync failure"):
        session_store.get_native_attempt_boundaries(root, "run", "attempt")
...
def test_native_only_reservation_recovers_journal_high_water(
    ...
    begin(root, "run", "attempt")
    assert (
        observe(root, receipt("run", "attempt", "entry", key("/native/high-water"))).chat_id == "c1"
    )
