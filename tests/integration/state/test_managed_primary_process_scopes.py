...
def test_reclaim_session_owned_scopes_is_idempotent_for_already_released(
    ...
    """reclaim_session_owned_scopes_for_chat skips scopes already marked released."""
    runtime_root, spawn_id = _create_primary_spawn(tmp_path)
    session_id = "session-1"
    scope = _scope(
        spawn_id,
    ...
    # First call — terminates and marks released.
    results = reclaim_session_owned_scopes_for_chat(runtime_root, "c1", grace_seconds=2.0)
    assert terminated_scope_ids == ["backend"]
    assert len(results) == 1

    # Second call — already released, must not call terminate again.
    terminated_scope_ids.clear()
    results = reclaim_session_owned_scopes_for_chat(runtime_root, "c1", grace_seconds=2.0)
    assert terminated_scope_ids == [], "terminate must not be called for already-released scopes"
    assert results == []
