"""Legacy collection parity through the pure shared replay builder."""
...
import meridian.lib.launch.continue_model_intent as continue_replay_module
...
from meridian.lib.launch.continue_replay import (
    build_continue_replay_contract as assemble_contract,
)
...
def build_continue_replay_contract(
    *, source, runtime_root=None, requested_model_override=None, **kwargs
):
    intent = continue_replay_module.collect_untracked_legacy_continue_intent(
        source=source,
        runtime_root=runtime_root,
        requested_model_override=requested_model_override,
        **kwargs,
    )
    return assemble_contract(source=source, intent=intent, **kwargs)
...
def _patch_intent_seams(
    ...
    *,
    ...
    monkeypatch.setattr(continue_replay_module, "get_model_selection", lambda *a, **k: recorded)
    monkeypatch.setattr(continue_replay_module, "get_initial_model_selection", lambda *a, **k: None)
    monkeypatch.setattr(continue_replay_module, "read_last_executed_model", lambda *a, **k: live)
    monkeypatch.setattr(continue_replay_module, "get_last_executed_model", lambda *a, **k: stored)
    monkeypatch.setattr(continue_replay_module, "record_model_observation", lambda *a, **k: True)
    monkeypatch.setattr(
...
def test_unroutable_observed_falls_back_to_recorded(
    ...
    monkeypatch.setattr(continue_replay_module, "run_mars_models_resolve", lambda *a, **k: None)
