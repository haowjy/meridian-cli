...
|------|---------|---------------|
| **A — Prompt rejection** | `history_shim_response_failure.jsonl` | Pi emitted failed `response` on stdout (`success: false`); drain records the error but report extraction used to fall through to cleanup lifecycle JSON. |
