...
def test_claude_tool_call_does_not_inherit_opencode_semantics() -> None:
    assert activity_transition(_event(HarnessId.CLAUDE, "tool_call")) is None


def test_opencode_turn_started_does_not_inherit_codex_semantics() -> None:
    assert activity_transition(_event(HarnessId.OPENCODE, "turn/started")) is None
...
def test_same_event_name_is_classified_independently_by_harness_id() -> None:
    ...
    assert normalize_event(claude_event) == ()
    assert normalize_event(opencode_event) == (
        ActivitySemanticEvent(state="turn_active"),
    )
...
def test_every_bundle_reserves_meridian_connection_close_semantics() -> None:
    for bundle in get_bundle_registry().values():
        assert MERIDIAN_CONNECTION_CLOSED_EVENT in bundle.semantics.event_classes
