...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="intent-acceptance-") as temporary:
        ...
        revision = hashlib.sha256(
            (source / "src/meridian/lib/launch/continue_replay.py").read_bytes()
            + (source / "src/meridian/lib/launch/continue_model_intent.py").read_bytes()
        ).hexdigest()
        print(
            "bootstrap",
            sys.version,
        ...
        try:
            ...
            assert not workload
        print("isolated replay acceptance passed", flush=True)
